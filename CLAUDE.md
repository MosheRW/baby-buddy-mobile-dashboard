...
- **Next — Phase 6:** server-backed timers via `/api/timers/`, reconciling persisted local timers with server ones on launch. Note `Timer.clean()` also runs `validate_time` on `start`, so the same `serverNow()` rule applies. Still unproven: the HA ingress path with a token, and the password/CSRF login end-to-end.
