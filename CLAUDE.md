...
- **Next — Phase 4:** the full Log Entry form — per-type field groups (diaper Pee/Poo toggles + poo colors, feeding kind→method + amount/duration + timer, medication dose/repeat/suggestions, temperature, tummy, sleep) driven by `formStore`, plus create/edit/delete against the mock store. See `docs/IMPLEMENTATION_PLAN.md`.
