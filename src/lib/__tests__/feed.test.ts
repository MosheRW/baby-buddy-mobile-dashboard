...
describe('foodTrend', () => {
  ...
  it('divides by active baseline days, not the full 7-day span', () => {
    // 700ml of baseline spread over only 2 active days → 350/day, not 100/day:
    // a logging gap must not drag the norm down.
    const entries: Entry[] = [
  ...
  it('reports a full-week basis without the "excluded" flag when every day is active', () => {
    const entries: Entry[] = [
      feeding('today', NOW - HOUR, 100),
      ...Array.from({ length: 7 }, (_, i) => feeding(`p${i}`, NOW - (i + 1) * DAY, 350)),
    ];
    const trend = foodTrend(entries, NOW);
    expect(trend.avgPerDay).toBe(350); // 2450 / 7
...
describe('foodTrendLabel', () => {
  it('reads as the prototype writes it when a full week is active', () => {
    const entries: Entry[] = [
      feeding('today', NOW - HOUR, 120),
      ...Array.from({ length: 7 }, (_, i) => feeding(`p${i}`, NOW - (i + 1) * DAY, 500)),
    ];
    expect(foodTrendLabel(foodTrend(entries, NOW))).toBe('120ml today vs 500ml/day (7d avg)');
  });

  it('names the active-day basis when some baseline days had no entries', () => {
    const entries: Entry[] = [
      feeding('today', NOW - HOUR, 100),
      feeding('p1', NOW - 1 * DAY, 350),
      feeding('p2', NOW - 2 * DAY, 350),
    ];
    expect(foodTrendLabel(foodTrend(entries, NOW))).toBe(
